...
def init_db():
    ...
    test_users = [
        ("admin", "1234", "İK Yöneticisi Ahsen", "IK_ADMIN"),
        ("calisan1", "1234", "Ahmet Yılmaz", "CALISAN"),
        ("stajyer1", "1234", "Mehmet Demir", "STAJYER")
    ]
    
    for u, p, fn, r in test_users:
        cursor.execute("SELECT id FROM users WHERE username = ?", (u,))
        if not cursor.fetchone():
            cursor.execute("INSERT INTO users (username, password, full_name, role) VALUES (?, ?, ?, ?)", (u, p, fn, r))
...
def verify_user(username, password):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT full_name, role FROM users WHERE username = ? AND password = ?", (username, password))
...
def create_conversation(username, title):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO conversations (username, title) VALUES (?, ?)", (username, title))
...
def get_user_conversations(username):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, title, created_at FROM conversations WHERE username = ? ORDER BY id DESC", (username,))
...
def get_user_requests(username):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT request_type, start_date, end_date, status, created_at FROM requests WHERE username = ? ORDER BY id DESC", (username,))
    rows = cursor.fetchall()
    conn.close()
    return rows
